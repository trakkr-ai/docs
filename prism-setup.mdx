...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/debug-cache') {
      if (!env.TRAKKR_KV) {
        return new Response('TRAKKR_KV not configured', {
          headers: { 'content-type': 'text/plain' },
          status: 500
        });
      }
      ...
      try {
        const data = await env.TRAKKR_KV.get(testKey);
        return new Response(JSON.stringify({
          kv_configured: true,
          cache_key: testKey,
    ...
    try {
      ...
      if (env.TRAKKR_KV) {
        try {
          const cached = await env.TRAKKR_KV.get(cacheKey);
          if (cached) {
...
async function logCrawlerVisit(env, visitData) {
  if (!env.ANALYTICS_KV) {
    console.log('ANALYTICS_KV not configured, skipping analytics');
    return;
  }
  ...
  updates.push(updateCounter(env.ANALYTICS_KV, overallKey, 0));
  
  // Daily stats
  const dailyKey = `stats:daily:${dateKey}:${crawlerType}`;
  updates.push(updateCounter(env.ANALYTICS_KV, dailyKey, ANALYTICS_CONFIG.RETENTION_DAYS + 10));
  
  // Weekly stats
  const weeklyKey = `stats:weekly:${weekKey}:${crawlerType}`;
  updates.push(updateCounter(env.ANALYTICS_KV, weeklyKey, ANALYTICS_CONFIG.RETENTION_DAYS + 10));
  
  // Monthly stats
  const monthlyKey = `stats:monthly:${monthKey}:${crawlerType}`;
  updates.push(updateCounter(env.ANALYTICS_KV, monthlyKey, ANALYTICS_CONFIG.RETENTION_DAYS + 30));
  
  // Last seen info
  const lastSeenKey = `last_seen:${crawlerType}`;
  updates.push(
    env.ANALYTICS_KV.put(lastSeenKey, JSON.stringify({
      timestamp,
      pathname,
      hostname,
  ...
  if (Math.random() < ANALYTICS_CONFIG.SAMPLE_RATE) {
    ...
    updates.push(
      env.ANALYTICS_KV.put(sampleKey, JSON.stringify(visitData), {
...
async function getCrawlerAnalytics(env, periodDays = 30, includeRaw = false) {
  if (!env.ANALYTICS_KV) {
    return { error: 'ANALYTICS_KV not configured' };
  }
  ...
  try {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      for (const crawler of crawlerTypes) {
        ...
        const count = await env.ANALYTICS_KV.get(key);
        if (count) {
    ...
    for (const pattern of AI_CRAWLER_PATTERNS) {
      ...
      const data = await env.ANALYTICS_KV.get(lastSeenKey);
      if (data) {
...
async function optimizeForAI(url, originalHTML, env) {
  ...
  try {
    ...
    console.log('KV namespace available?', !!env.TRAKKR_KV);
    
    // Internal API endpoint - do not modify
    const apiUrl = 'https://prism-html-830752870576.us-east1.run.app';
    ...
    if (!env.TRAKKR_KV) {
      console.error('CRITICAL: KV namespace TRAKKR_KV is not bound!');
    ...
    try {
      await env.TRAKKR_KV.put(cacheKey, JSON.stringify(cacheData), {
        expirationTtl: 7 * 24 * 60 * 60 // 7 days TTL
      });
      
      console.log('✓ Successfully cached AI-optimized content');
      console.log('  Cache key:', cacheKey);
      console.log('  Data size:', JSON.stringify(cacheData).length, 'bytes');
      
      // Verify the write worked
      const verify = await env.TRAKKR_KV.get(cacheKey);
