...
<Info>
  This feature requires an API key to get set up, which is only available on the [Scale](https://learn.trakkr.ai/plans#available-plans) plan.
</Info>
...
1. [**Trakkr API Key**](https://trakkr.ai/api-keys): A valid API key starting with `sk_live_`
